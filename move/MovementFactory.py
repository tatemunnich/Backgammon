...
def getMovesForDoubles(color, distance_dict, starting_loc, root):
    ...
    if board.numBar(color) > 0:
        ...
        try:
            ...
            move.apply(scratch)
            # apply die 1
            move_node = MoveNode(move, scratch, parent=root)
...
def generateMoves2(board: Board, color: str, dice: Dice, verbose=False):
    root = MoveNode("root", board_after=board, color=color, dice=dice)
    ...
    return set(PreOrderIter(root, filter_=lambda node: node.is_leaf and node.depth == height))
