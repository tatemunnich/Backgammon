...
def generate_moves(board, color, dice, verbose=False):
    ...
    for location in initial_locations:
        first_1 = buildMovement(board, color, die_1, location, return_board=True)
        if first_1:
            ...
            if len(locations_1) == 0:
                moves.add(Move(board, color, dice, [first_movement_1]))
                return moves
            for location_1 in locations_1:
                second_movement_1 = buildMovement(board_after_1, color, die_2, location_1)
                # print(second_movement_1)
                if second_movement_1:
                    can_move_2_second = True
                    # print("trying to add move " + str(Move(board, color, dice, [first_movement_1, second_movement_1])))
                    moves.add(Move(board, color, dice, [first_movement_1, second_movement_1]))

        first_2 = buildMovement(board, color, die_2, location, return_board=True)
        if first_2:
            ...
            if len(locations_2) == 0:
                moves.add(Move(board, color, dice, [first_movement_2]))
                return moves
            for location_2 in locations_2:
                second_movement_2 = buildMovement(board_after_2, color, die_1, location_2)
                if second_movement_2:
                    can_move_1_second = True
                    # print("trying to add move " + str(Move(board, color, dice, [first_movement_2, second_movement_2])))
                    moves.add(Move(board, color, dice, [first_movement_2, second_movement_2]))
    ...
    if len(moves) == 0:
        if not (can_move_1_first or can_move_2_first):
            ...
            for location in initial_locations:
                first_movement = buildMovement(board, color, die_1, location)
                if first_movement:
            ...
            for location in initial_locations:
                first_movement = buildMovement(board, color, die_2, location)
                if first_movement:
                    moves.add(Move(board, color, dice, [first_movement]))
            ...
            for location in initial_locations:
                first_movement = buildMovement(board, color, max(dice.getDice()), location)
                if first_movement:
                    moves.add(Move(board, color, dice, [first_movement]))
